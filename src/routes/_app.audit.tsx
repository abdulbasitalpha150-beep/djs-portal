...
//       try {
//         const payload = await apiFetch<{ users: AuthActivity[] }>("/api/audit-logs", { method: "GET" });
...
//       return new Date(user.lastLogin) > new Date(user.lastLogout) ? user.lastLogin : user.lastLogout;
//     }
...
//       <PageHeader title="Authentication Activity" description="Latest login/logout activity for all users." />
//       {loading ? (
//         <div className="rounded-lg border border-border bg-card p-8 text-center text-sm text-muted-foreground">Loading activity…</div>
//       ) : items.length === 0 ? (
//         <div className="rounded-lg border border-border bg-card p-8 text-center text-sm text-muted-foreground">No authentication activity yet.</div>
//       ) : (
//         <DataTable
//           rows={items}
//           columns={[
//             { head: "User", cell: (u) => <span className="font-medium">{u.name}</span> },
//             {
//               head: "Action",
//               cell: (u) => (
//                 <Badge variant={u.action === "Login" ? "success" : u.action === "Logout" ? "destructive" : "outline"}>
//                   {u.action}
//                 </Badge>
//               )
//             },
//             { head: "IP Address", cell: (u) => <span className="font-mono text-xs text-muted-foreground">{u.ipAddress}</span> },
//             { head: "Last Login", cell: (u) => u.lastLogin ? <span className="text-sm">{fmtDate(u.lastLogin)}</span> : "—" },
//             {
//               head: "Last Active",
//               cell: (u) => {
//                 const lastActivity = getLastActivityDate(u);
//                 return lastActivity ? <span className="text-sm">{relative(lastActivity)}</span> : "—"
//               }
//             },
//           ]}
//         />
